...
async function GET() {
  ...
  <url>
    ...
    const generalArticlesSitemap = articlesData.map((article)=>`
  <url>
    ...
    const serviceArticlesSitemap = serviceArticlesData.map((article)=>`
  <url>
    <loc>${baseUrl}/articles/${article.slug}</loc>
    <lastmod>${article.lastModified}</lastmod>
    <changefreq>${article.changefreq}</changefreq>
    ...
    const projectsSitemap = projects.map((project)=>{
        const images = project.mediaItems?.filter((media)=>media.type === 'IMAGE').slice(0, 5) // تقليل عدد الصور لتحسين الأداء
        .map((media)=>`
      <image:image>
        ...
        const videos = project.mediaItems?.filter((media)=>media.type === 'VIDEO').slice(0, 3) // تقليل عدد الفيديوهات
        .map((media)=>`
      <video:video>
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e2a08c8c._.js.map